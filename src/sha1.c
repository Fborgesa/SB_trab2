...
int generate_sha1(sha1_info *context, const uint8_t *original_message, unsigned length,  uint8_t sha1_hash[HASH_SIZE]){
    ...
    for(;length--;){
        context->msg_block[context->msg_block_index++] = (*original_message & MASK_LEAST);
